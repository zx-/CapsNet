...
def _example_dataset(x):
    ...
    x_dat = x_dat.map(tf.to_float)
    x_dat = x_dat.map(lambda img: img / 255)
    x_dat = x_dat.map(lambda img: tf.expand_dims(img, axis=-1))
...
def _target_dataset(y, classes=NUM_CLASSES):
    ...
    return y_dat.map(lambda x: tf.one_hot(x, classes))
